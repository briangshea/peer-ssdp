...
Peer.prototype.reply = function (headers, address, callback) {
    var _headers = {};
    //HOST is not defined in as a header field for M-SEARCH responses
    //_headers['HOST'] = headers['HOST'] || SSDP_HOST;

    // Force Order of headers as per UPNP Device Architecture
    _headers['CACHE-CONTROL'] = headers['CACHE-CONTROL'] || MAX_AGE;
    _headers['DATE'] = headers['DATE'] || new Date().toUTCString();
    _headers['EXT'] = headers['EXT'] || "";
    _headers['LOCATION'] = headers['LOCATION'] || "";
    _headers['SERVER'] = headers['SERVER'] || "";
    _headers['ST'] = headers['ST'] || "";
    _headers['USN'] = headers['USN'] || "";
    _headers['BOOTID.UPNP.ORG'] = (this.bootid_upnp_org++);
    //if( headers.hasOwnProperty('CONFIGID.UPNP.ORG')) {
    //  _headers['CONFIGID.UPNP.ORG'] = headers['CONFIGID.UPNP.ORG'];
    //}

    var processMessageCallback = function (networkInterfaceAddress) {
        var msg = new Buffer(serialize("HTTP/1.1 " + TYPE_200_OK, _headers, networkInterfaceAddress));
